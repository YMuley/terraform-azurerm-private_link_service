resource "azurerm_private_link_service" "private_link_service" {
  ...
  dynamic "nat_ip_configuration" {
    ...
    content {
      ...
      subnet_id                  = flatten([for subnet in var.subnet_output : subnet.id if contains(each.value.subnet_name, subnet.name) == true]) # subnet_id = element([for subnet in var.subnet_output : subnet.id if subnet.name == each.value.subnet_name], 0)
