resource "azurerm_private_link_service" "private_link_service" {
  ...
  load_balancer_frontend_ip_configuration_ids = each.value.frontend_ip_configuration_ids //flatten([for load_balancer in var.load_balancer_output : load_balancer.id if contains(each.value.load_balancer_name, load_balancer.name) == true])
